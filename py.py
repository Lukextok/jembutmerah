...
def get_user_profile(user_id):
    ...
    # Gabungkan data dari berkas gmail.txt lokal jika ada
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"{display}\n\n"
        "⚡ *Sistem:* Input Gmail langsung via Bot Aktif! (Hapus pesan otomatis dinonaktifkan)."
    )
...
async def inline_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "none": return
        ...
        await query.message.reply_text("📝 Silakan kirim data akun dengan format:\n\n`Gmail` `SandiAplikasi`\n\n*(Sandi boleh pakai spasi asli bawaan Google atau digabung)*", parse_mode='Markdown')
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "WAITING_SENDER":
        user_states[user_id] = None
        # Pembersihan cerdas: pisahkan bagian pertama sebagai email, sisanya digabungkan dan dibuang spasinya
        parts = text_input.split(maxsplit=1)
        if len(parts) >= 2:
            gmail = parts[0].strip()
            # Buang spasi di dalam sandi aplikasi 16 digit bawaan Google
            app_pass = parts[1].replace(" ", "").strip()
            
            if not any(acc['gmail'] == gmail for acc in profile["accounts"]):
                profile["accounts"].append({"gmail": gmail, "pass": app_pass})
                update_user_profile(user_id, profile)
            await update.message.reply_text(f"✅ Email Pengirim `{gmail}` sukses disimpan di memori bot!", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text("❌ Format salah! Harap pisahkan email dan sandi dengan spasi.", reply_markup=get_main_keyboard())
        return
...
async def kirim_email_tunggal(num, acc, target_email, pesan_pilihan, status_msg):
    ...
    try:
        ...
        # 💡 FITUR HAPUS IMAP SEKARANG DIBUANG AGAR TIDAK ADA ERROR ATAU DELAY
...
async def process_file_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not profile.get("accounts"): 
        await update.message.reply_text("⚠️ Pasukan Gmail kosong! Gunakan menu Setting untuk menambahkan akun.")
        return
