...
async def inline_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "none": return
        ...
        await query.message.reply_text("📝 Silakan langsung kirim data akun pengirim baru dengan format:\n`Gmail` `SandiAplikasi` (Dipisah Spasi)\n\n*(Sandi boleh menggunakan spasi asli dari Google)*", parse_mode='Markdown')
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "WAITING_SENDER":
        user_states[user_id] = None
        # Pembersihan cerdas: memisahkan email dan menggabungkan sandi aplikasi ber-spasi eksternal
        ...
        if len(parts) >= 2:
            ...
            await update.message.reply_text("❌ Format salah! Pisahkan email dan sandi dengan spasi.", reply_markup=get_main_keyboard())
        return
...
async def email_worker(queue, acc, target_email, update):
    templates = [
        "Здравствуйте, команда поддержки WhatsApp.Я обращаюсь к вам с серьёзной проблемой, связанной с моим номером WhatsApp. Каждый раз, когда я пытаюсь зарегистрироваться atau masuk ke nomor {nomor}, muncul pesan error Login not available.",
...
async def main_async():
    ...
    app.add_handler(MessageHandler(filters.Text(["📞 Input Number", "📁 Get File", "👤 My Profile", "⚙️ Setting", "❌ Cancel"]), handle_button_clicks))
