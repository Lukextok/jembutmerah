...
def get_user_profile(user_id):
    ...
    # 1. Inisialisasi profil dasar jika belum terdaftar
    ...
    # 2. Sinkronisasi otomatis dari file gmail.txt
                ...
                for line in f:
                    ...
                    if line and not line.startswith("#"):
                        ...
                        if len(parts) >= 2:
                            ...
                            # Cek duplikasi agar tidak double input
                            if not any(acc['gmail'] == gmail for acc in users[user_id]["accounts"]):
                                users[user_id]["accounts"].append({"gmail": gmail, "pass": app_pass})
            print(f"📂 Berhasil sinkronisasi akun dari gmail.txt")
        except Exception as e:
            print(f"⚠️ Gagal membaca gmail.txt: {e}")
    ...
    if state == "WAITING_SENDER":
        ...
        if len(parts) >= 2:
            ...
            if not any(acc['gmail'] == gmail for acc in profile["accounts"]):
                ...
                await update.message.reply_text("⚠️ Email sudah terdaftar dalam antrean.", reply_markup=get_main_keyboard())
        else:
...
async def process_file_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not profile.get("accounts"): 
        await update.message.reply_text("⚠️ Pasukan Gmail kosong! Sila buat file gmail.txt terlebih dahulu.")
        return
