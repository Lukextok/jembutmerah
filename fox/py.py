    ...
    if query.data == "none": return
        ...
        if 0 <= index < len(accounts):
            ...
            if success:
                ...
                update_user_profile(user_id, profile)
                await query.edit_message_reply_markup(reply_markup=get_settings_inline_kb(profile))
                await query.message.reply_text(f"🗑️ Email `{removed['gmail']}` sukses terhapus permanen dari GitHub.")
            else:
                await query.message.reply_text("❌ Gagal sinkronisasi hapus ke GitHub Actions.")
        ...
        if len(parts) >= 2:
            ...
            if success:
                if not any(acc['gmail'] == gmail for acc in profile["accounts"]):
                    ...
                    update_user_profile(user_id, profile)
                await update.message.reply_text(f"✅ Email Pengirim `{gmail}` Berhasil disimpan Permanen di Cloud GitHub!", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text("❌ Gagal menyimpan ke GitHub Actions. Periksa izin token.", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text("❌ Format salah! Kirim ulang dengan spasi antara email dan sandi.", reply_markup=get_main_keyboard())
        return
